    ...
    for test_file in config['input_data']['dataset']:
        ...
        for epoch in range(0, config['training']['epoches']):
            ...
            if(epoch%config['training']['validation_interval']==0):
                ...
                # Adjust learning rate based on cumulative rewards
                total_rewards += reward
                if total_rewards > 0:
                    lr *= 1.05  # Increase learning rate by 5%
                elif total_rewards < 0:
                    lr *= 0.95  # Decrease learning rate by 5%

                # Update optimizer with new learning rate
                for param_group in optimizer.param_groups:
                    param_group['lr'] = lr

                print(f"Reward: {reward} | Cumulative Reward: {total_rewards} | Adjusted Learning Rate: {lr}")
