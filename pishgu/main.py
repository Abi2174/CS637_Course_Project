...
# Load configuration
with open('configs\pedestrian\\birds_eye.yaml', "r") as file:
...
# Determine delimiter based on dataset
if config['input_data']['dataset'] == ["VIRAT_ActEV"]:
    delim = 'space'
else:
    delim = 'tab'

# Set ADE and FDE thresholds for reward calculation
ade_threshold = config['training'].get('ade_threshold', 0.67)
fde_threshold = config['training'].get('fde_threshold', 1.25)
...
if config['training']['train']:
    ...
    for test_file in config['input_data']['dataset']:
        ...
        # Training Loop with Reward-based Reinforcement
        for epoch in range(config['training']['epoches']):
            total_rewards = 0  # Accumulate rewards over an epoch
            
            # Training step
            losses = ut.train(model, train_loader, optimizer, device, obs_step=config['input_data']['observed_steps'])
            
            # Validation step at specified intervals
            if epoch % config['training']['validation_interval'] == 0:
                ade, fde = ut.test(model, val_loader, device)
                print(f"ADE: {ade}, FDE: {fde}, Epoch: {epoch}")

                # Reward mechanism based on ADE and FDE thresholds
                if ade < ade_threshold and fde < fde_threshold:
                    reward = 1  # Positive reward for low error
                else:
                    reward = -1  # Negative reward for high error
                
                # Accumulate total rewards for this epoch
                total_rewards += reward

                # Print reward information
                print(f"Reward: {reward} | Cumulative Reward: {total_rewards}")

                # Update best model based on ADE
                if ade < best_info[0]:
                    best_info[0] = ade
                    best_info[1] = fde
                    best_info[2] = epoch
                    if config['training']['save_model']:
                        model_path = f"models/{saveFolder}/{test_file}.pt"
                        torch.save(model.state_dict(), model_path)

            print(f"End of Epoch {epoch} | Total Rewards: {total_rewards}")
...
# Testing Mode
    ...
    for test_file in config['input_data']["dataset"]:
        ...
        model.load_state_dict(torch.load(os.path.join(config['training']['model_dir'], test_file) + ".pt", map_location='cpu'), strict=False)
        ...
        if config['input_data']['subject'] == 'vehicle':
            rmse_batch = torch.full((25, 1), 0.0).squeeze(dim=1).to(device)
            number_of_traj = 0
        ...
        for batch in test_loader:
            ...
            pred_traj = pred_traj.reshape(pred_traj.shape[0], config['input_data']['prtediction_step'],
                                          config['input_data']['points_per_position']).detach()
            ...
            if config['input_data']['subject'] == 'vehicle':
                rmse_batch += ut.rmse(pred_traj_real, pred_traj_gt, mode='raw')
                number_of_traj += 1
    ...
    avg_fde = sum(data_st[2] for data_st in res) / len(res)
    avg_ade = sum(data_st[1] for data_st in res) / len(res)
