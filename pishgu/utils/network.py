...
# CBAM and Spatio-Temporal Attention Modules
################################################################################################################################
class SpatioTemporalAttention(nn.Module):
    def __init__(self, input_dim):
        super(SpatioTemporalAttention, self).__init__()
        self.temporal_fc = nn.Linear(input_dim, input_dim)
        self.temporal_softmax = nn.Softmax(dim=1)
        
        self.spatial_fc = nn.Conv2d(input_dim, 1, kernel_size=1)
        self.spatial_softmax = nn.Softmax(dim=-1)

    def forward(self, x):
        # Temporal attention
        temporal_attn = self.temporal_softmax(self.temporal_fc(x))
        
        # Spatial attention
        spatial_attn = self.spatial_softmax(self.spatial_fc(x))
        
        # Combine attentions
        x = x * temporal_attn.unsqueeze(-1).unsqueeze(-1) * spatial_attn
        return x
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        for head in self.attention_heads:
            attn_weights = self.softmax(head(x))
            head_outputs.append(attn_weights * x)
...
class AttentionGINConv(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_heads=4):
        ...
        self.gin = GINConv(
            ...
            Seq(  # Assuming nn2 is needed, we use another Sequential layer as a placeholder
                Lin(hidden_dim, hidden_dim),
                ReLU(),
                Lin(hidden_dim, hidden_dim)
            )
    ...
    def forward(self, x, edge_index):
        ...
        x = x.unsqueeze(0)  # Add batch dimension for MHA
        x, _ = self.mha(x, x, x)
        return x.squeeze(0)
